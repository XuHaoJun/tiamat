...
export function fetchForumBoards(page = 1, limit = 10, sort = '-popularityCounter', reqConfig = {}) {
  return (dispatch) => {
    return callApi(`forumBoards?sort=${sort}&page=${page}&limit=${limit}`, 'get', null, reqConfig).then((res) => {
...
export function fetchForumBoardById(_id) {
  return (dispatch) => {
    return callApi(`forumBoards/${_id}`).then((res) => {
...
export function addForumBoardRequest(forumBoard) {
  return (dispatch) => {
    return callApi('forumBoards', 'post', {forumBoard}).then((res) => {
      dispatch(addForumBoard(res.forumBoard));
      return res.forumBoard;
    }).catch((err) => {
      return Promise
