...
class LinkableListItem extends ListItem {
  ...
  render() {
    ...
    return (<ListItem {...other} href={to} onTouchTap={this.handleTouchTap} onClick={this.preventDefault}/>);
  }
}
...
export function getRootWikiGroupTreeListItemsHelper2(rootWikiGroupTree, path = [], getHrefFunc = defaultGetHrefFunc) {
  if (Map.isMap(rootWikiGroupTree)) {
    return rootWikiGroupTree.map((node, k) => {
      const finalPath = path.concat(k);
      const nestedItems = getRootWikiGroupTreeListItemsHelper2(node, finalPath, getHrefFunc);
      const href = getHrefFunc(pathToRootWikiGroupTree(finalPath, false));
      const value = href;
      return (<LinkableListItem to={href} value={value} key={value} primaryText={k} nestedItems={nestedItems}/>);
    })
      .toList()
      .toJS();
    ...
    return rootWikiGroupTree.map((leaf) => {
      ...
      const value = href;
      return (<LinkableListItem to={href} value={value} key={value} primaryText={leaf}/>);
    }).toJS();
  } else {
    return null;
  }
...
export function getRootWikiGroupTreeListItems2(rootWikiGroupTree, getHrefFunc) {
  return getRootWikiGroupTreeListItemsHelper2(rootWikiGroupTree, undefined, getHrefFunc);
}

export function getRootWikiGroupTreeListItemsIter(rootWikiGroupTree, path = [], getHrefFunc = defaultGetHrefFunc) {
  if (Map.isMap(rootWikiGroupTree)) {
    ...
    const value = href;
    return (<LinkableListItem to={href} value={value} key={value} primaryText={name} nestedItems={nestedItems}/>);
  } else if (ImmutableList.isList(rootWikiGroupTree)) {
...
export function getRootWikiGroupTreeListItems(rootWikiGroupTree, getHrefFunc) {
  return getRootWikiGroupTreeListItemsIter(rootWikiGroupTree, undefined, getHrefFunc);
}
...
class RootWikiGroupTreeList extends React.Component {
  ...
  render() {
    ...
    const rootWikiGroupTreeListItems = getRootWikiGroupTreeListItems(rootWikiGroupTree, this.getWikisHref);
    let defaultValue;
