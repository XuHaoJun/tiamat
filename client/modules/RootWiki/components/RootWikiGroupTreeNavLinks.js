...
export function getRootWikiGroupTreeNavLinksHelper(rootWikiGroupTree, path = [], getHrefFunc = () => '', hrefs = []) {
  if (Map.isMap(rootWikiGroupTree)) {
    return rootWikiGroupTree.map((node, k) => {
      ...
      const newHrefs = hrefs.concat([
        [k, href]
      ]);
      return getRootWikiGroupTreeNavLinksHelper(node, finalPath, getHrefFunc, newHrefs);
    })
      .toList()
      .flatten(1);
...
export function getRootWikiGroupTreeNavLinks(rootWikiGroupTree, getHrefFunc = () => '') {
  return getRootWikiGroupTreeNavLinksHelper(rootWikiGroupTree, undefined, getHrefFunc).toJS();
}
...
const Delimeter = (props) => {
  ...
  return (
    <span style={delimeterStyle} {...props}>{delimeter}</span>
  );
};
...
class RootWikiGroupTreeNavLinks extends React.Component {
  ...
  render() {
    ...
    if (!rootWikiGroupTree) {
      ...
      return (
        <Link to={href}>未分類</Link>
      );
    }
    // /forumBoards/:forumBoardId/rootWikis/:rootWikiId/wikis
    const navLinks = getRootWikiGroupTreeNavLinks(rootWikiGroupTree, this.getWikisHref);
    return (
      <span>
        {
          navLinks.map((line, lineIndex) => {
            const lineDOM = line.map(([
              name, href
            ], index, array) => {
              const link = (
