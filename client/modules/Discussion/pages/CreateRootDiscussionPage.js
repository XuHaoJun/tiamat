...
import {setCreateRootDiscussionPageForm, addDiscussionRequest} from '../DiscussionActions';
  ...
  componentWillUnmount() {
    if (this.formComponent) {
      ...
      if (form) {
        ...
        this
          .props
          .dispatch(setCreateRootDiscussionPageForm(forumBoardId, form));
      }
    }
  }
      ...
      this
        ...
        .dispatch((dispatch) => {
          return Promise
            ...
            .then(() => dispatch(setCreateRootDiscussionPageForm(forumBoardId, null)))
...
function mapStateToProps(store, props) {
  ...
  const initForm = ui.getIn(['CreateRootDiscussionPage', 'forms', forumBoardId]);
  const rootWikiId = forumBoard
    ? forumBoard.get('rootWiki')
    : '';
  const semanticRules = rootWikiId
    ? getSemanticRules(store).filter(ele => ele.get('rootWikiId') === rootWikiId).sortBy(ele => ele.get('name').length)
    : Set();
