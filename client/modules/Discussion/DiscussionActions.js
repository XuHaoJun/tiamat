...
export const SET_CREATE_ROOT_DISCUSSION_PAGE_FORM = 'SET_CREATE_ROOT_DISCUSSION_PAGE_FORM';
...
export function fetchRootDiscussions(forumBoardId, _opts, reqConfig = {}) {
  ...
  return (dispatch) => {
    return callApi(url, 'get', null, reqConfig).then((res) => {
...
export function fetchDiscussions(forumBoardId = '', parentDiscussionId = '') {
  return (dispatch) => {
    return callApi(`discussions?forumBoardId=${forumBoardId}&parentDiscussionId=${parentDiscussionId}`).then((res) => {
...
export function fetchDiscussion(id) {
  return (dispatch) => {
    return callApi(`discussions/${id}`).then((res) => {
...
export function addDiscussionRequest(discussion) {
  return (dispatch) => {
    return callApi('discussions', 'post', {discussion}).then((res) => {
      dispatch(addDiscussion(res.discussion));
