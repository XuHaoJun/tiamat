...
const useScrollMiddleware = useScroll((prevRouterProps, routerProps) => {
  if (prevRouterProps) {
    if (prevRouterProps.location.pathname === routerProps.location.pathname && routerProps.location.action === 'REPLACE') {
      return false;
    }
...
export default function App(props) {
  ...
  const routerKey = process.env.NODE_ENV !== 'production' ? Math.random() : undefined;
  return (
