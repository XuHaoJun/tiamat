  ...
  const schema = {
    nodes: {
      image: (props) => {
        ...
        const active = state.isFocused && state
          .selection
          .hasEdgeIn(node);
        const src = node
          .data
        ...
        return (<Img alt={src} style={style} src={src} className={className} {...props.attributes}/>);
      },
      mention: Mention,
      table: (props) => {
        const {node, state} = props;
      ...
      table_cell: (props) => {
        ...
        return (
          <TableRowColumn style={style}>{props.children}</TableRowColumn>
        );
      },
      paragraph: Paragraph,
      link: Link,
      ...
      // document.
      {
        ...
        validate: (document) => {
          const lastNode = document
            .nodes
            .last();
          return lastNode && lastNode.isVoid
            ? true
            : null;
        },
        normalize: (transform, document) => {
