...
let serialize = (content) => {
  const rawContent = Map.isMap(content)
    ? content.toJS()
    : emptyContent.toJS();
  const state = Slate
    .Raw
    .deserialize(rawContent, {terse: true});
  return state;
};
...
class Editor extends React.Component {
  ...
  constructor(props) {
    ...
    const {enterScrollWindowPlugin, suggestionsPlugin, tablePlugin, softBreakPlugin} = defaultPlugins;
    const plugins = [tablePlugin, enterScrollWindowPlugin, suggestionsPlugin, softBreakPlugin];
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!is(nextProps.rawContent, this.props.rawContent)) {
      ...
      if (Map.isMap(rawContent)) {
        nextState.state = Slate
          .Raw
          .deserialize(rawContent.toJS(), {terse: true});
      }
    }
    ...
    if (nextProps.semanticReplaceMode !== this.state.semanticReplaceMode) {
      nextState = Object.assign(nextState, this.toggleSemanticReplaceHelper(nextProps.semanticReplaceMode));
    }
  ...
  onKeyDown = (e, data, state) => {
    if (data.isMod) {
      ...
      if (data.key === 'enter') {
        ...
        const shouldBreakout = breakoutRules.some(rule => state.startBlock.type === rule);
        if (shouldBreakout) {
          e.preventDefault();
          return state
            .transform()
  ...
  onClickBlock = (e, type) => {
    ...
    if (type !== 'bulleted-list' && type !== 'numbered-list') {
      ...
      if (isList) {
        ...
        transform.insertNodeByKey(state.blocks.get(0).get('key'), 1, Block.create({
          type: isActive
            ? DEFAULT_NODE
            : type
        }));
  ...
  renderMarkButton = (type, icon) => {
    ...
    return (
      <span style={styles.container} onMouseDown={onMouseDown} data-active={isActive}>
        <span style={styles.icon} className="material-icons">{icon}</span>
      </span>
    );
  };
  ...
  renderEditor = () => {
    ...
    const isTable = tablePlugin
      .utils
      .isSelectionInTable(state);
    const {SuggestionPortal} = suggestionsPlugin;
    const styles = getStyles();
    const {readOnly} = this.state;
    let editorStyle = readOnly
      ? null
      : styles.editor;
    let editorContainerStyle = readOnly
      ? null
    ...
    return (
      <div style={editorContainerStyle}>
        ...
        <Slate.Editor
          ...
          autoComplete={false}
          autoCorrect={false}/>
        <SuggestionPortal state={this.state.state} suggestions={this.state.suggestions}/>
      </div>
    );
  };

  render() {
    const ContainerComponent = this.shouldFullScreen()
      ? Portal
      : Div;
    return (
      <ContainerComponent>
