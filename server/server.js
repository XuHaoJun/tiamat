...
const renderFullPage = (html, initialState) => {
  ...
  return `
    ...
    : ''}
        ...
        <meta charset="utf-8" />
        ${process.env.NODE_ENV === 'production' && googleAnalyticsConfig['google-site-verification'] ?
        `<meta name='google-site-verification' content='${googleAnalyticsConfig['google-site-verification']}'/>`
        : ''}
        <script>
          (function() {
      ...
      : `<div>${html}</div>`}</div>
        <script>
          window.__INITIAL_STATE__ = ${serializeJavascript(initialState, {isJSON: true})};
          ${process.env.NODE_ENV === 'production'
        ? `//<![CDATA[
...
app.use((req, res, next) => {
  match({
    ...
    return fetchComponentData(store, renderProps.components, renderProps.params, renderProps.location.query).then(() => {
...
server.listen(appConfig.server.port, (error) => {
  if (!error) {
    console.log(`Tiamat is running on port: ${appConfig.server.port}! Build something amazing!`); // eslint-disable-line
  }
});
